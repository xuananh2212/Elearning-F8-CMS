...
import {
  ...
  requestUpdateCourse,
} from "../middlewares/course.middewares";
const initialState = {
  loading: false,
  courses: [],
  topicDetail: null,
...
export const courseSlices = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(requestGetAllCourseByTeacher.fulfilled, (state, action) => {
      state.courses = action.payload.courses;
      state.loading = false;
    });
    ...
    builder.addCase(requestDeleteManyCourse.fulfilled, (state, action) => {
      state.courses = state.courses.filter(
        (course) => !action.payload?.courseIds.includes(course.id)
      );
      state.loading = false;
    });
  },
