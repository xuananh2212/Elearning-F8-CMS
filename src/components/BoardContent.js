import {
  requestUpdateSortLesson,
  requestUpdateSortLessonDifferentTopic,
} from "@/store/middlewares/lesson.middlewares";
...
export default function BoardContent({ course, setCurrentAction }) {
  ...
  const [oldTopicWhenDraggingLesson, setOldTopicWhenDraggingLesson] =
    useState(null);
  ...
  const handleMoveLessonBetweenDifferentTopics = (
    ...
    if (nextActiveTopic) {
      nextActiveTopic.Lessons = nextActiveTopic.Lessons.filter(
        (lesson) => lesson.id !== activeDraggingLessonId
      );
    ...
    if (nextOverTopic) {
      ...
      nextOverTopic.Lessons = nextOverTopic.Lessons.filter(
        (lesson) => lesson.id !== activeDraggingLessonId
      );
      //  thêm cái lesson đang kéo vào overColumn theo vị trí đang index mới;
      nextOverTopic.Lessons = nextOverTopic.Lessons.toSpliced(
        newLessonIndex,
        0,
        {
          ...activeDraggingLessonData,
      ...
      nextOverTopic.Lessons = nextOverTopic.Lessons.map((lesson, index) => ({
        ...lesson,
        sort: index + 1,
      }));
  ...
  const findTopicByLessonId = (lessonId) => {
    return topicDetail.find((topic) =>
      topic?.Lessons?.map((lesson) => lesson.id).includes(lessonId)
    );
  };
  const dropAnimation = {
  ...
  const handOnDragStart = async (result) => {
    ...
    setActiveDragItemType(
      result?.active?.data?.current?.topic_id
        ? ACTIVE_DRAG_TTEM_TYPE.LESSON
        : ACTIVE_DRAG_TTEM_TYPE.TOPIC
    );
  ...
  const handOnDragEnd = async (result) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_TTEM_TYPE.LESSON) {
      ...
      if (oldTopicWhenDraggingLesson?.id !== overTopic?.id) {
        ...
        try {
          const oldLessonIndex = oldTopicWhenDraggingLesson?.Lessons?.findIndex(
          ...
          let dndLessons = arrayMove(
            oldTopicWhenDraggingLesson?.Lessons,
            oldLessonIndex,
            newLessonIndex
          );
          dndLessons = dndLessons.map((dndLesson, index) => ({
            ...dndLesson,
            sort: index + 1,
          }));
    ...
    if (activeDragItemType === ACTIVE_DRAG_TTEM_TYPE.TOPIC) {
      if (active?.id !== over?.id) {
        const oldTopicIndex = topicDetail?.findIndex(
          ({ id }) => id === active?.id
        );
        const newTopicIndex = topicDetail?.findIndex(
          ({ id }) => id === over?.id
        );
