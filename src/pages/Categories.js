...
export default function Categories() {
  ...
  const categoriesTreeData = useSelector(
    (state) => state.category.categoriesTreeData
  );
  const validateCategory = useSelector(
    (state) => state.category.validateCategory
  );
  ...
  const handleDeleteManyCategory = async () => {
    try {
      const response = await dispatch(
        requestDeleteManyCategory({ categoryIds: selectedRowKeys })
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "parentId",
      render: (id) =>
        getParentCategories(id, categories).map(({ name, id }, index) => {
          return (
            <Tag color="blue" key={id} className="ml-1">
              {name}
            </Tag>
          );
        }),
      ...
      >
        ...
        >
          <Form.Item
            className="mb-1"
            name="name"
            label={<h3 className="text-[16px]">Tên danh mục</h3>}
          >
          ...
          <Form.Item
            name="parentId"
            label={<h3 className="text-[16px]">Danh mục cha:</h3>}
          >
            <TreeSelect
              showSearch
              style={{ width: "100%" }}
